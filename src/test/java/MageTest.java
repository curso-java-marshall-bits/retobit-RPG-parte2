...
public class MageTest {
    ...
    void testCastSpellMethod() {
        ...
        int initialMana = mage.mana;
        int initialTargetHealth = target.health;

        mage.castSpell(target);

       assertTrue(mage.mana < initialMana, "castSpell() debería consumir 10 de mana.");
        assertEquals(initialMana - 10, mage.mana, "castSpell() no consume la cantidad correcta de mana.");
        ...
        assertEquals(5, lowManaMage.mana, "castSpell() no debería consumir mana si es insuficiente y no lanzar el hechizo.");
    }
    ...
    void testHealMethod() {
        ...
        int initialMana = healer.mana;
        int allyHealthBeforeHeal = injuredAlly.health;

        healer.heal(injuredAlly);

        assertTrue(healer.mana < initialMana, "heal() debería consumir 15 de mana.");
        assertEquals(initialMana - 15, healer.mana, "heal() no consume la cantidad correcta de mana.");
